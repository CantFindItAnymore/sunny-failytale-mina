...
Page({
	...
	onLoad: function (options) {
		User.getUserInfo().then(res => {
			...
			let babyInfo = {
				...
				weight: res.weight,
				underwearSize: res.underwearSize,
			}
			this.setData({
				userInfo,
				babyInfo,
	...
	confirmDatePicker(e) {
		this.data.activeRole === 'user' &&
			this._changeUserInfo('birthday', timestampToTime(e.detail))
		this.data.activeRole === 'baby' &&
			this._changeBabyInfo('birthday', timestampToTime(e.detail))
		this.setData({
			datePickerShow: false,
		})
	},
    ...
    console.log(8, this.data.babyInfo)
		...
		User.auth({
			height: this.data.babyInfo.height || null,
			shoesSize: this.data.babyInfo.shoesSize || null,
			weight: this.data.babyInfo.weight || null,
      underwearSize: this.data.babyInfo.underwearSize || null,
